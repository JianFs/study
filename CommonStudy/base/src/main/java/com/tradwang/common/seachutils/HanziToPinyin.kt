 ...
 */
internal class HanziToPinyin private constructor(private val mHasChinaCollator: Boolean) {
    ...
    class Token {
        ...
        internal constructor(type: Int, source: String, target: String) {
            this.type = type
            this.source = source
            this.target = target
        }
        ...
        companion object {
            ...
            internal const val LATIN = 1
            internal const val PINYIN = 2
    ...
    private fun getToken(character: Char): Token {
        ...
        if (offset < 0) {
            ...
            while (begin <= end) {
                offset = (begin + end) / 2
                val uniHan = Character.toString(UNI_HANS[offset])
    ...
    operator fun get(input: String): ArrayList<Token> {
        ...
        // Go through the input, init a new token when
    ...
    companion object {
        ...
        val instance: HanziToPinyin
            get() = synchronized(HanziToPinyin::class.java) {
                if (sInstance != null) {
